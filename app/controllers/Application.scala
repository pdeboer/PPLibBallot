...
object Application extends Controller {
	...
	def storeAnswer = Action { request =>
		request.session.get("TurkerID").map { user =>
			try {
				val questionId = requestData(request)("questionId").mkString.toLong

				if (isUserAllowedToAnswer(questionId, UserDAO.findByTurkerId(user).get.id.get)) {

					val answer: JSONObject = JSONObject.apply(requestData(request).map(m => {
						(m._1, m._2.mkString(","))
					}))
	...
	def requestData(request: Request[AnyContent]): Map[String, Seq[String]] = {
		request.queryString
